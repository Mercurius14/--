FIXED_IP = [2, 6, 3, 1, 4, 8, 5, 7] # 初始置换
FIXED_EP = [4, 1, 2, 3, 2, 3, 4, 1] # 扩展置换
FIXED_IP_INVERSE = [4, 1, 3, 5, 7, 2, 8, 6] # 逆初始置换
FIXED_P10 = [3, 5, 2, 7, 4, 10, 1, 9, 8, 6] # 密钥置换
FIXED_P8 = [6, 3, 7, 4, 8, 5, 10, 9] # 密钥置换
FIXED_P4 = [2, 4, 3, 1]  # 密钥置换



S0 = [[1, 0, 3, 2],
      [3, 2, 1, 0],
      [0, 2, 1, 3],
      [3, 1, 0, 2]]

S1 = [[0, 1, 2, 3],
      [2, 3, 1, 0],
      [3, 0, 1, 2],
      [2, 1, 0, 3]]

# KEY = '0111111101'

# 置换操作
def permutate(original, fixed_key):
    new = ''
    for i in fixed_key:
        new += original[i - 1]
    return new

# 左半部分
def left_half(bits):
    return bits[:int(len(bits)/2)]

# 右半部分
def right_half(bits):
    return bits[int(len(bits)/2):]

# 左移操作
def shift(bits):
    rotated_left_half = left_half(bits)[1:] + left_half(bits)[0]
    rotated_right_half = right_half(bits)[1:] + right_half(bits)[0]
    return rotated_left_half + rotated_right_half


# 密钥1生成
def key1(key):
    return permutate(shift(permutate(key, FIXED_P10)), FIXED_P8)

# 密钥2生成
def key2(key):
    return permutate(shift(shift(permutate(key, FIXED_P10))), FIXED_P8)

# 异或操作
def xor(bits, key):
    new = ''
    for bit, key_bit in zip(bits, key):
        new += str(((int(bit) + int(key_bit)) % 2))
    return new

# S盒查找
def lookup_in_sbox(bits, sbox):
    row = int(bits[0] + bits[3], 2)
    col = int(bits[1] + bits[2], 2)
    return '{0:02b}'.format(sbox[row][col])

# F函数
def f_k(bits, key):
    L = left_half(bits)
    R = right_half(bits)
    bits = permutate(R, FIXED_EP)
    bits = xor(bits, key)
    bits = lookup_in_sbox(left_half(bits), S0) + lookup_in_sbox(right_half(bits), S1)
    bits = permutate(bits, FIXED_P4)
    return xor(bits, L)

# 加密
def encrypt(plain_text,key):
    bits = permutate(plain_text, FIXED_IP)
    temp = f_k(bits, key1(key))
    bits = right_half(bits) + temp
    bits = f_k(bits, key2(key))
    return permutate(bits + temp, FIXED_IP_INVERSE)

# 解密
def decrypt(cipher_text,key):
    bits = permutate(cipher_text, FIXED_IP)
    temp = f_k(bits, key2(key))
    bits = right_half(bits) + temp
    bits = f_k(bits, key1(key))
    return permutate(bits + temp, FIXED_IP_INVERSE)

# test 测试用
def test():
    plain_text = input("请输入明文：")
    key = input("请输入密钥：")
    cipher_text = encrypt(plain_text,key)
    print("加密密文",cipher_text)
    print("密文解密",decrypt(cipher_text,key))


# test()
